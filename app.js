...
      errorMessage = "⏱️ Λήγξη χρόνου. Η απάντηση κράτησε πολύ. Ξαναδοκιμάστε.";
    }
    
...
  try {
    const res = await fetch("https://gpai-chatbot-backend.onrender.com/api/chat", {
      method: "POST",
...
      body: JSON.stringify({
        ...
        { 
        role: "system", 
        content: "Είστε ανώτερος Έλληνας νομικός σύμβουλος εξειδικευμένος στον AI Act και απαντάτε αποκλειστικά στα ελληνικά με θεσμικό ύφος."
        },
        ...chatHistory,
...
    if (data.reply) {
      // the backend may include the model in its JSON payload
      return { text: data.reply, model: data.model || "" };
    } else if (data.error) {
      throw new Error(data.error);
    } else {
      throw new Error("Ανεπιθύμητη απόκριση από την υπηρεσία");
    }
  } catch (error) {
    clearTimeout(timeoutId);
    
...
      throw new Error("network: Μη δυνατή η σύνδεση με τον διακομιστή");
    }
    
    throw error;
  }
